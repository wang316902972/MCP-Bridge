...
class TestSSEBridge:
    ...
    async def test_sse_handshake(self, http_client):
        ...
        async with http_client.stream("GET", "/mcp-server/sse/") as response:
            if response.status_code == 200:
                ...
                assert "text/event-stream" in content_type or chunks
    ...
    async def test_sse_jsonrpc_over_sse(self, http_client):
        ...
        async with http_client.stream("GET", "/mcp-server/sse/") as response:
            if response.status_code == 200:
                ...
                content_type = response.headers.get("content-type", "")
                assert "text/event-stream" in content_type or message_count > 0
    ...
    async def test_sse_tools_list(self, http_client):
        ...
        response = await http_client.post(
            "/mcp-server/sse/", json={"jsonrpc": "2.0", "method": "tools/list", "id": 1}
        )
    ...
    async def test_sse_authentication(self, http_client, auth_headers):
        ...
        async with http_client.stream(
            "GET", "/mcp-server/sse/", headers=headers
        ) as response:
            # 根据认证配置返回不同状态码
            if response.status_code == 200:
                print("✅ SSE 认证成功")
    ...
    async def test_sse_reconnect_capability(self, http_client):
        """测试 SSE 重连能力"""
        # 第一次连接
        async with http_client.stream("GET", "/mcp-server/sse/") as response1:
            status1 = response1.status_code

        # 立即重连
        async with http_client.stream("GET", "/mcp-server/sse/") as response2:
    ...
    async def test_sse_error_handling(self, http_client):
        ...
        # 发送无效请求
        async with http_client.stream(
            "GET", "/mcp-server/sse/", params={"invalid": "param"}
        ) as response:
            # 应该优雅地处理错误
...
class TestMCPClientIntegration:
    ...
    async def test_desktop_client_compatibility(self, http_client):
        ...
        # 验证端点结构符合 MCP 客户端期望
        endpoints = ["/mcp-server/sse/", "/v1/mcp/"]
